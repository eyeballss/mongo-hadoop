...
public class PageRank_NoQuery {
	...
	private static class CleaningUpMap extends Mapper<Text, Text, Text, BSONWritable> {
		public void map(Text key, Text value, Context context) throws IOException, InterruptedException {
			String rankAndTo = value.toString();
			int index = rankAndTo.indexOf(SEPARATOR);

			BasicBSONObject output = new BasicBSONObject();
			String onlyRank = rankAndTo.substring(0, index);
			output.put("rank", onlyRank);

			BSONWritable rank = new BSONWritable();
			rank.setDoc(output);
	...
	private static Job rankingUpdateJobSetting(Configuration conf, String[] args, String from, String to)
			throws IOException {
	...
	private static Job cleaningUpJobSetting(Configuration conf, String[] args, String from, String to)
			throws IOException {
		MongoConfigUtil.setOutputURI(conf, "mongodb://" + args[0] + to);
